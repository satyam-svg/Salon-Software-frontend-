...
const PackageManagement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        editingPackage ? "Failed to update package" : "Failed to create package"
      );
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    try {
      setLoading(true);
      await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}api/packages/delete/${id}`
      );
      await fetchPackages();
    } catch (err) {
            ...
            >
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                  <FiGitBranch />
                  <span>
                    Branch Limit: {formatBranchLimit(pkg.branchLimit)}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-4">
                    <select
                      ...
                      value={
                        formData.isUnlimitedBranches ? "unlimited" : "limited"
                      }
                      onChange={(e) =>
                        setFormData({
                          ...formData,
